...
def std_atm(altitude: float, units: str, print_output: bool = False) -> dict:
    """
    Standard Atmosphere Calculator

    Inputs:
        altitude: Altitude in desired units from {m, km, ft}
    ...
    """
    unit_conversion = {'m': 1/1000, 'ft': 0.0003048, 'km': 1}
    
    if units in unit_conversion:
        altitude *= unit_conversion[units]
    else:
        print(f'Unit argument: "{units}" in the std_atm function is not valid.\n' \
              'The input should be one of: {"km", "m", "ft"}')
        return None
