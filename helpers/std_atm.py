...
class Atmosphere(BaseModel):
    ...
    density: Optional[float] = np.nan
    viscosity: Optional[float] = np.nan
    speed_of_sound: Optional[float] = np.nan
    mach: Optional[float] = np.nan
    reynolds_number: Optional[float] = np.nan
    dynamic_pressure: Optional[float] = np.nan
    ...
    def evaluate(
        ...
        atm.pressure = P_inf
        atm.temperature = T_inf
        atm.density = rho_inf
        atm.viscosity = mu_inf
        atm.speed_of_sound = np.sqrt(1.4 * 287 * T_inf)
        atm.mach = mach
        atm.reynolds_number = (rho_inf * mach * atm.speed_of_sound) / mu_inf
        atm.dynamic_pressure = (1/2) * rho_inf * (mach * atm.speed_of_sound)**2

        # TODO: Implement more freestream characteristics (q,a, etc.)
        return atm

if __name__ == "__main__":
    alt = 10
    units = 'km'
    Atmosphere.evaluate(altitude=alt, units=units, print_output=True)